...
public class SteelInABottleItem extends Item implements ProjectileItem {
    ...
    public InteractionResult use(final Level level, final Player player, final InteractionHand hand) {
        ...
        if (level instanceof ServerLevel serverLevel) {
            Projectile.spawnProjectileFromRotation(ThrownSteelInABottle::new, serverLevel, itemStack, player, -20.0F, 0.7F, 1.0F);
        }
    ...
    public Projectile asProjectile(final Level level, final Position position, final ItemStack itemStack, final Direction direction) {
        return new ThrownSteelInABottle(level, position.x(), position.y(), position.z(), itemStack);
    }
